...
void gen(int i, int mask, int nextMask, vi& nextMasks) {
    if (i == n) {
    ...
    if (mask & (1 << i)) {
        gen(i + 1, mask, nextMask, nextMasks);
    } else {
        gen(i + 1, mask, nextMask | (1 << i), nextMasks);
        if (i + 1 < n && !(mask & (1 << (i + 1))))
            gen(i + 2, mask, nextMask, nextMasks);
    }
}
void test() {
    ...
    for (int col = 0; col < m; col++) {
        for (int mask = 0; mask < maxMask; mask++) {
            ...
            vi nextMasks;
            gen(0, mask, 0, nextMasks);
            for (auto nMask : nextMasks) {
                dp[col + 1][nMask] = modAdd(dp[col + 1][nMask], dp[col][mask]);
            }
        }
    }
    cout << dp[m][0] << nl;
